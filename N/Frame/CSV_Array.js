...
function csv2array(data, delimeter) {                    // Retrieve the delimeter
  ...
  while (c != eof) {                                    // skip whitespaces
    while (c == ' ' || c == '\t' || c == '\r') {
      c = data.charAt(++i); // read next char
    }
    
    var value = "";                                     // get value
    if (c == '\"') {                                    // value enclosed by double-quotes
      c = data.charAt(++i);
      
      do {
        if (c != '\"') {                                // read a regular character and go to the next character
          value += c;
          c = data.charAt(++i);
        }
        
        if (c == '\"') {                                // check for escaped double-quote
          var cnext = data.charAt(i+1);
          if (cnext == '\"') {                          // this is an escaped double-quote 
            value += '\"';                              // Add a double-quote to the value, and move two characters ahead
            i += 2;
            c = data.charAt(i);
          }
        }
    ...
    if (array.length <= row) {                         // add the value to the array
      array.push(new Array());
    }
    array[row].push(value);
    
    while (c == ' ' || c == '\t' || c == '\r') {       // skip whitespaces
      c = data.charAt(++i);
    }

    if (c == delimeter) {                              // to the next column
      col++; }
    else if (c == newline) {                           // to the next row
      col = 0;
      row++; }
    else if (c != eof) {                               // unexpected character
      throw "Delimiter expected after character " + i;
    }
    
    c = data.charAt(++i);                              // go to the next character
  }  

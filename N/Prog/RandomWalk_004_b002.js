...
function RandomWalk_004() {
  ...
  function init_layer() {     
    layerSimL  = new CLayer('sim','#ffffff',10);
    layerSim   = layerSimL.get_obj();   
    layerObjL  = new CLayer('obj','#ffffff',10);
    layerObj   = layerObjL.get_obj(); 
  }
  /*------------------------------------------------------- init_canvas ---*/
  ...
  function init_canvas()    {
    ...
    for(var i=1;i<=nPoint;i++) {
      cS_path[i] = new CCanvas (layerSim,i+1,'#' 
  ...
  function init_obj()      { 
    ...
    for(var i=1;i<=nPoint;i++) {
      cb_path[i]  = makeCb   (layerObj, 40,200 + 20*(i-1), 15,15,
                              "Path " + i,true ,60,15);
    } 
  }
  /*----------------------------------------------------- init_listener ---*/

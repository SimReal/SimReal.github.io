...
function Stat_PI_SquareCircle() {
  ...
  function init_layer() {     
    layerSimL  = new CLayer('sim',10,0,0,'#ffffff');  layerSim = layerSimL.get_obj();   
    layerObjL  = new CLayer('obj',10,0,0,'#ffffff');  layerObj = layerObjL.get_obj(); 
  }
  /*------------------------------------------------------------------------------------------ init_canvas ---*/

  function init_canvas()    {
    cS_axis    = new CCanvas (layerSim,1,0,0,'#000000');
    cS_path    = new Array(nPoint+1);
    for(var i=1;i<=nPoint;i++) {
      cS_path[i] = new CCanvas (layerSim,i+1,0,0,'#' + Math.floor(16777215*Math.random()).toString(16));
    }
    cS_square = new CCanvas(layerSim,2,0,0,'#000000');
    cS_circle = new CCanvas(layerSim,3,0,0,'#000000');
    cS_vector = new CCanvas(layerSim,4,0,0,'#FF00FF');
  }
  /*-------------------------------------------------------------------------------------------- init_axis ---*/

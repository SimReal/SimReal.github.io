...
function Stat_PI_SquareCircle() {
  ...
  function init_canvas()    {
    cS_axis    = new CCanvas (layerSim,1,'#000000',0,0);
    cS_path    = new Array(nPoint+1);
    for(var i=1;i<=nPoint;i++) {
      cS_path[i] = new CCanvas (layerSim,i+1,'#' + Math.floor(16777215*Math.random()).toString(16),0,0);
    }
    cS_square = new CCanvas(layerSim,2,'#000000',0,0);
    cS_circle = new CCanvas(layerSim,3,'#000000',0,0);
    cS_vector = new CCanvas(layerSim,4,'#FF00FF',0,0);
  }
  /*-------------------------------------------------------------------------------------------- init_axis ---*/
